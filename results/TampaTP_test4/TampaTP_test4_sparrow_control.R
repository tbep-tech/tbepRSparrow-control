...
# SPARROW URL:  http://water.usgs.gov/nawqa/sparrow/
 ...
 # window in RStudio (located next to the "Run" button). 
  ...
  diagnostic_timeSeriesPlots<-"yes"
  ...
  add_plotlyVars<-c("waterid","rchname","staid","mapping_waterid") 
  showPlotGrid<-"no"

  #dynamic mapping settings
  #set years of data to include or select "all" to map all years 
  #or select "mean" to map an average of all years for each season selected
 # map_years<-NA #for static model
map_years<-c(2001,2002)
#map_years<-"mean"  #setting both map_years and map_seasons = "mean" aggregates all data onto single map
#map_years<-"all"
  
  #set seasons of data to include or select "all" to map all seasons 
  #or select "mean" to map an average of all seasons for each year
  #seasons can be defined with strings or numbers, but should match 
  #data1 variable class
  #map_seasons<-NA #for static model
#  map_seasons<-c("winter","summer","spring","fall")
#map_seasons<-"mean"
  map_seasons<-"all"
 #map_seasons<-"winter"
  
  #Set maximum number of maps per page from 1-4
  #fewer maps may appear per page if the mapPageGroupBy selection contains
  #less available data for maps than mapsPerPage selection
  mapsPerPage<-4
  
  #set grouping variable for maps on a page, select "season","year", or NA for no group
  #   GroupBy year Example: If mapsPerPage<-4 and the year 2001 has only 3 seasons of data,
  #                         setting the mapPageGroupBy <-"year" will result in a 3-panel
  #                         map for 2001 followed by a 4 panel plot for 2002 if 4 seasons 
  #                         exist for 2002
  #   GroupBy season Example: Grouping by season will result in only 1 season being displayed 
  #                           per page.  If mapping only 2 years (2001 and 2002), 3 2-panel maps
  #                           would be generated for each of the 3 seasons that are present in 2001,
  #                           and 1 1-panel map for the season that was missing from 2001 but 
  #                           present in 2002
  #   No Grouping Example: If mapsPerPage<-4 and mapPageGroupBy<-NA, the result will be that
  #                        every page contains 4 maps and then the last page of maps would 
  #                        have any remainder, so 3 maps from 2001 would be mixed with 
  #                        1 map from 2002 to form a 4 panel map and then the last page would 
  #                        contain a 3 panel map of the remaining seasons in 2002, assuming
  #                        2002 is the last year being mapped.  
  ...
  #select file to input forecasted values for scenario
  forecast_filename<-"forecastScenario.csv"
  #forecast_filename<-NA
  #use_sparrowNames = TRUE if forecast data file uses sparrowNames, FALSE if data1UserNames are used
  ...
  scenario_sources <- NA
  scenario_sources <- c("urban_Seas","logAirTemp") 
 ...
 # scenario_factors <- c(0.20)# order consistent with order of 
  ...
  scenario_map_list <- c("ratio_total")
  ###########################################
...
path_master<-'./RSPARROW_master'
  ...
  run_id<-"TampaTP_test4"
  
  # #Select year(s) and season(s) to model
  # model_years<-c(2003,2014)
  # #model_years<-NA
  # 
  # model_seasons<-c("winter","spring","summer","fall")
  # #model_seasons<-NA
