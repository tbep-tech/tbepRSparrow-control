...
#   
 ##years and seasons
     ...
     year<-sapply(timestep, function(x) as.numeric(as.character(substr(x,1,4))))
  season<-sapply(timestep, function(x) ifelse(substr(x,5,6)=="01","winter",
                 ifelse(substr(x,5,6)=="02","spring",
                        ifelse(substr(x,5,6)=="03","summer","fall"))))
...
hucTest<-sample.int(10,length(waterid),replace=TRUE)
  landTest<-sample.int(5,length(waterid),replace=TRUE)
  ############################################################
  ...
  S_logAirTemp<-ifelse(year==2014 & season=="winter",0.2,1)
  S_logPPT<-ifelse(year==2014 & season=="winter",0.1,1)
